...
public class BoundsPopupMenuListener implements PopupMenuListener {
    ...
    protected void customizePopup(BasicComboPopup popup) {
        ...
        if (popupAbove) {
            ...
            popup.setLocation(location.x, Math.max(location.y - height, 0));
        } else {
            int height = comboBox.getPreferredSize().height;
            popup.setLocation(location.x, Math.min(location.y + height - 1, screen.height));
            popup.setLocation(location.x, Math.min(location.y + height, screen.height));
        }
    }
